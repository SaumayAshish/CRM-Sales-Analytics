...
| FR-65 | An opportunity's `probability` field, already stage-defaulted on every stage transition, shall also be directly editable by its owning Rep, their Manager, or Admin via `PATCH /opportunities/{id}`, so a rep can flag a specific deal as higher- or lower-confidence than its stage's default. | BR-16 (weighted pipeline value) | Editing probability on an open opportunity updates its weighted value everywhere it's read (Kanban card, `vw_pipeline_summary`, `vw_win_probability_buckets`) without requiring a stage change. |
| FR-66 | The system shall expose a company-wide quarterly revenue target (`GET/PATCH /company-targets/current`), Admin-editable, distinct from a rep's individual quota, backing the Pipeline Coverage Ratio KPI. | BR-24 | A non-Admin `PATCH` is rejected with 403; `GET` auto-creates a current-quarter row (carrying the prior quarter's target forward) rather than erroring when a new quarter has no row yet. |

**Traceability note:** FR-46–FR-66 slot into the existing chain (`BR-xx → FR-xx → US-xx → UAT-xx`); corresponding user stories and UAT cases are added to `User_Stories.md` / `UAT_Test_Scripts.md` as part of Phase 3/6 delivery, not deferred.
